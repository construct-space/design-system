...
const groups = computed(() => [...new Set(pages.map(p => p.group))])
</script>
  ...
  <nav class="w-56 shrink-0 border-r flex flex-col fixed top-0 left-0 bottom-0 overflow-y-auto"
    ...
    <div v-for="group in groups" :key="group" class="px-3 mb-2">
      ...
      <button
        ...
        :style="{
          color: currentPage === page.id ? 'var(--c-accent)' : 'var(--c-muted)',
          background: currentPage === page.id ? 'color-mix(in srgb, var(--c-accent) 10%, transparent)' : 'transparent',
        }"
